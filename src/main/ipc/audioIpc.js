...
const { LICENSE_FEATURES, enforceLicensedFeature, toLicenseErrorPayload } = require("../licensing/featureGuard");
...
function registerAudioIpc() {
  ...
  ipcMain.handle("audio:import", async (evt, payload) => {
    try {
      ...
      if (err?.licenseError) return toLicenseErrorPayload(err);
  ...
  ipcMain.handle("audio:transcribe", async (_evt, payload) => {
    try {
      ...
      if (!err?.licenseError && payload?.audioImportId) {
        try {
